...
def evaluate_exact_new(pred_a, true_a, eps=1e-11):
    """
    计算精确度、召回率和 F1 分数。
    :param pred_a: 预测的接触图 (二维矩阵)展平
    :param true_a: 真实的接触图 (二维矩阵)展平
    :param eps: 防止分母为零的极小值
    :return: 精确度、召回率和 F1 分数
    """
    tp_map = torch.sign(torch.Tensor(pred_a) * torch.Tensor(true_a))  # 真正例
    tp = tp_map.sum().item()  # 真正例的数量
    pred_p = torch.sign(torch.Tensor(pred_a)).sum().item()  # 预测为正的总数
    true_p = torch.Tensor(true_a).sum().item()  # 真实为正的总数
    fp = pred_p - tp  # 假正例的数量
    fn = true_p - tp  # 假负例的数量
    # 计算真负例 (TN)
    total_elements = len(pred_a)  # 总元素数
    tn = total_elements - (tp + fp + fn)  # 真负例的数量
    ...
    # 计算 MCC
    mcc = ((tp * tn - fp * fn) + eps_tensor) / torch.sqrt(
        (tp + fp) * (tp + fn) * (tn + fp) * (tn + fn) + eps_tensor)  # MCC
...
def contact_f1(ref_batch, pred_batch, L, th=0.5, reduce=True, method="triangular"):
    ...
    for ref, pred, l in zip(ref_batch, pred_batch, L):
        ...
        if method == "triangular":
            f1,mcc1,precision,recall = f1_triangular(ref, pred>th)
        elif method == "shift":
            ref = mat2bp(ref)
            pred = mat2bp(pred)
        ...
        f1_list.append(f1)
        mcc_list.append(mcc1)
        precision_list.append(precision)
        recall_list.append(recall)

    if reduce:
        return tr.tensor(f1_list).mean().item(),tr.tensor(mcc_list).mean().item(),tr.tensor(precision_list).mean().item(),tr.tensor(recall_list).mean().item()
    else:
...
def f1_triangular(ref, pred):
    ...
    precision, recall, f1, mcc1 = evaluate_exact_new(pred, ref)

    return f1,mcc1,precision,recall
